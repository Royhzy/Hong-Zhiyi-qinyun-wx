...
Page({
  ...
  //滑动
    ...
    goSearch(){                                   //获取搜索数据
        ...
        if(key){
            var that = this
            wx.request({
                url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/querykey?catogory=小提琴&key='+key,
    ...
    getList(){                         //获取下拉数据
        var that = this
        wx.request({
            url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/queryfun?catogory=小提琴',
            success: function (res) {
  ...
  xiaoliang(){             //销量按钮（只有降序）
    ...
    if(key){
        var that = this
        wx.request({
            url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/keySaleSort?catogory=小提琴&key='+key,
            success: function (res) {
                that.setData({ 
                    list: res.data 
                })
        ...
        var that = this
        wx.request({
            url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/salesort?catogory=小提琴',
            success: function (res) {
                that.setData({ 
                    list: res.data 
                })
  ...
  jiage(){              //价格按钮
    ...
    if(priceSo==true){
        if(key){
            var that = this
            wx.request({
                url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/keyPriceSort?catogory=小提琴&key='+key+'&sc=desc',
                success: function (res) {
                    that.setData({ 
                        list: res.data 
                    })
            ...
            var that = this
            wx.request({
                url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/pricesort?catogory=小提琴&sc=desc',
                success: function (res) {
                    that.setData({ 
                        list: res.data 
                    })
        ...
        if(key){
            var that = this
            wx.request({
                url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/keyPriceSort?catogory=小提琴&key='+key+'&sc=asc',
                success: function (res) {
                    that.setData({ 
                        list: res.data 
                    })
            ...
            var that = this
            wx.request({
                url: 'http://www.qinyunbs.com:8090/wxapi/Yueqi/pricesort?catogory=小提琴&sc=asc',
                success: function (res) {
                    that.setData({ 
                        list: res.data 
                    })
